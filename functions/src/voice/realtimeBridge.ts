 ...
 *
 * ⚠️  DEPLOY TO CLOUD RUN, NOT CLOUD FUNCTIONS
 *
 * Firebase Cloud Functions do not support long-lived WebSocket connections.
 * This file is a standalone WebSocket server that should be deployed to
 * Google Cloud Run (or any Node.js host that supports persistent WebSockets).
 *
 * DEPLOYMENT:
 *   cd functions
 *   gcloud run deploy voice-realtime-bridge \
 *     --source . \
 *     --platform managed \
 ...
 *
 * HOW IT WORKS:
 *   Twilio Media Stream (μ-law 8kHz audio) → this bridge → OpenAI Realtime API (PCM 24kHz)
 *   OpenAI Realtime API (PCM 24kHz audio)  → this bridge → Twilio Media Stream
 *
 * Latency: ~300ms (vs ~1-2s for the stateless Gather approach in webhooks.ts)
 *
 * TWILIO TWIML TO USE THIS BRIDGE:
 *   <Connect>
 *     <Stream url="wss://YOUR_CLOUD_RUN_URL/media-stream?agentId=...&businessId=..." />
 *   </Connect>
 */
...
const server = http.createServer((_req, res) => {
  res.writeHead(200).end('Voice Realtime Bridge — healthy')
})
...
wss.on('connection', (twilioWs, req) => {
  ...
  // When OpenAI connection is ready, configure the session
  openaiWs.on('open', () => {
    ...
    try {
      ...
      if (msg.event === 'media' && openaiWs.readyState === WebSocket.OPEN) {
        openaiWs.send(JSON.stringify({
          ...
          audio: msg.media.payload,   // already μ-law base64 from Twilio
        }))
